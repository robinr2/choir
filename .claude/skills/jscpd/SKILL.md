...
npx --prefix server jscpd --reporters ai <path>

# With ignore patterns
npx --prefix server jscpd --reporters ai --ignore "**/node_modules/**,**/dist/**" <path>

# Scope to specific formats
npx --prefix server jscpd --reporters ai --format "javascript,typescript" <path>

# Second pass, noisier: copies that only differ in names and values (Type-2, "renamed").
# Review each hit before acting on it.
npx --prefix server jscpd --reporters ai --ignore-identifiers --min-tokens 70 <path>
...
npx --prefix server jscpd --reporters ai --max-gap-lines 1 --similarity 0.85 <path>

# Where to refactor first: clone list plus a hotspot summary
npx --prefix server jscpd --reporters ai --summary <path>

# Most complex files only, without clone detection
npx --prefix server jscpd --complexity --reporters ai <path>
```
...
```bash
npx --prefix server jscpd --reporters ai --ignore-identifiers <path>                      # function a(x) {…} matches function b(y) {…}
npx --prefix server jscpd --reporters ai --ignore-identifiers --ignore-literals <path>    # …and 10 matches 25, 'dev' matches 'prod'
npx --prefix server jscpd --reporters ai --ignore-annotations <path>                      # @Override / @Deprecated no longer split a clone
```
...
```bash
npx --prefix server jscpd --reporters ai --max-gap-lines 2 <path>      # a copy with up to 2 inserted/changed lines becomes one clone
npx --prefix server jscpd --reporters ai --similarity 0.8 <path>       # JS/TS functions with ≥80% shared syntax-tree structure
```
...
npx --prefix server jscpd --reporters ai --summary --no-tips <path>

# Rank by complexity instead of tokens, top 5 lists
npx --prefix server jscpd --reporters ai --summary --summary-by complexity --summary-top 5 <path>
```
...
npx --prefix server jscpd --reporters ai --cross-formats "javascript,typescript" <path>

# Preset covering javascript, jsx, typescript, tsx
npx --prefix server jscpd --reporters ai --cross-formats "js-ts" <path>

# Multiple groups are separated by ";"
npx --prefix server jscpd --reporters ai --cross-formats "javascript,typescript;css,scss" <path>
```

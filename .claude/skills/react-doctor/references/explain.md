...
```bash
npx --prefix web react-doctor rules explain --cwd web react-doctor/no-array-index-as-key
```
...
```bash
npx --prefix web react-doctor web --verbose --diff
```

## Commands

```bash
npx --prefix web react-doctor rules list --cwd web                         # every rule + its effective severity
npx --prefix web react-doctor rules list --cwd web --configured            # only what your config changed
npx --prefix web react-doctor rules list --cwd web --category Performance   # filter by category
npx --prefix web react-doctor rules explain --cwd web <rule>               # why it matters + how to configure
npx --prefix web react-doctor rules disable --cwd web <rule>               # rule never runs
npx --prefix web react-doctor rules enable --cwd web <rule>                # turn back on at its recommended severity
npx --prefix web react-doctor rules set --cwd web <rule> warn              # off | warn | error
npx --prefix web react-doctor rules category --cwd web "React Native" off   # whole category
npx --prefix web react-doctor rules ignore-tag --cwd web design            # skip a rule family (design, test-noise, …)
npx --prefix web react-doctor rules unignore-tag --cwd web design
```
